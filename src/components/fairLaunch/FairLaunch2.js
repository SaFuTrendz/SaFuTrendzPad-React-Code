...
var valNetState = '';
class FairLaunchPad2 extends Component {
	constructor(props) {
		super(props);
	...
	validateField(fieldName, value) {
		...
		switch (fieldName) {
			...
			case 'maxBuy':
				...
				break;
			case 'from':
				fromValid = !isEmpty(value) ? true : false;
				fieldValidationErrors.from = fromValid ? '' : 'must have start time.';
				break;
			case 'to':
				toValid = !isEmpty(value) ? true : false;
								...
								<div className="lead2" style={{ marginTop: '100px' }}>
									<form>
										...
										>
											...
											<ButtonToolbar>
												<Link to={this.state.formValid ? '/FairLaunch3' : '#'}>
													<button
...
export default connect(mapStateToProps, { setWhiteListData, setReduxValue })(FairLaunchPad2);
