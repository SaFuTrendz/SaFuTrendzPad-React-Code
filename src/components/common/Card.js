...
function Card(props) {
	...
	const onStartTimeSet = () => {
		if (localStorage.getItem('isAuthenticated') === 'ture') {
			setStartClass('active');
			setEndClass('');
			minSelect1 = props.startAlarmTime5;
			minSelect2 = props.startAlarmTime15;
	...
	const onFavorite = () => {
		if (localStorage.getItem('isAuthenticated') === 'true' && localStorage.getItem('userAddress') !== null) {
			var data = {
				userAddress: localStorage.getItem('userAddress'),
				tokenAddress: props.tokenAddress,
				favorite: true
			};
			axios.post(`/api/alarm/addFavorite`, data).then((res) => console.log(res)).catch((err) => console.log(err));
		} else {
			alert('Please connect Wallet');
		}
	};
		...
		<div className="card-list">
			...
			<div className="card-body p-5 p-lg-6 pt-0 pt-lg-0">
				...
				<Row>
					<Col md={15} xs={15}>
						<div className="has-text-left">
							<b style={{ fontSize: '23px' }}>{props.privateSale ? 'Private' : 'Presale'} Launch</b>
						</div>
					</Col>
					<Col md={9} xs={9}>
						<div className="has-text-right">
